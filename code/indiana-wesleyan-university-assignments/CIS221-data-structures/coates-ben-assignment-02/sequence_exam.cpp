...
// **************************************************************************
// int test8( )
//   Performs some basic tests of operators == and !=
// **************************************************************************

int test8( )
{
  // Creates five sequences for testing
  std::cout << "Creating four sequences A,B,C, D, E & F" << std::endl << std::endl;
  sequence testa, testb, testc, testd, teste, testf;
	
  // Creates sequence A with 10, 20 and 30
  std::cout << "Sequence A cotains 10, 20, & 30" << std::endl;
  testa.attach(10);
  testa.attach(20);
  testa.attach(30);
	
  // Creates sequence B with 10, 20 and 30
  std::cout << "Sequence B cotains 10, 20, & 30" << std::endl;
  testb.attach(10);
  testb.attach(20);
  testb.attach(30);

  // Creates sequence C with 5, 10, and 15
  std::cout << "Sequence C cotains 5, 10, & 15" << std::endl;
  testc.attach(5);
  testc.attach(10);
  testc.attach(15);
	
  // Creates sequence A with 10, 20, 30 and 40
  std::cout << "Sequence D cotains 10, 20, 30 & 40" << std::endl;
  testd.attach(10);
  testd.attach(20);
  testd.attach(30);
  testd.attach(40);
	
  // Creates two empty sequences
  std::cout << "Sequence E cotains nothing" << std::endl;
  std::cout << "Sequence F cotains nothing" << std::endl << std::endl;

  // Test two sequences that are empty
  std::cout << "Testing to see if sequence E is equal to sequence F!" << std::endl;
  if (teste.operator!=(testf))
    {
      std::cout << "Test Failed: Sequence E is not equal to sequence F!" << std::endl;
      return 0;
    }
  std::cout << "Sequence E is equal to sequence F!...Passed" << std::endl;
	
  // Test two sequences that are equal
  std::cout << "Testing to see if sequence A is equal to sequence B!" << std::endl;
  if (testa.operator!=(testb))
    {
      std::cout << "Test Failed: Sequence A is not equal to sequence B!" << std::endl;
      return 0;
    }
  std::cout << "Sequence A is equal to sequence B!...Passed" << std::endl;
	
	
  // Test two sequences that have the same amount of items but are not equal
  std::cout << "Testing to see if sequence A is not equal to sequence C!" << std::endl;
  if (testa.operator==(testc))
    {
      std::cout << "Test Failed: Sequence A is equal to sequence C!" << std::endl;
      return 0;
    }
  std::cout << "Sequence A is not equal to sequence C!...Passed" << std::endl;
	
  // Test two sequences that have different amounts of items but the same items in the first 3 spots.
  std::cout << "Testing to see if sequence A is not equal to sequence D!" << std::endl;
  if (testa.operator==(testc))
    {
      std::cout << "Test Failed: Sequence A is equal to sequence D!" << std::endl;
      return 0;
    }
  std::cout << "Sequence A is not equal to sequence D!...Passed" << std::endl << std::endl;
  // Test Passed
  std::cout << "All tests of this eigth function have been passed" << std::endl;
  std::cout << "END OF TEST 8." << std::endl << std::endl;
